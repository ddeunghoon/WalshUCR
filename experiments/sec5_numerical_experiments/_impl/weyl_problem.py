...
from walsh_ucr.benchmarks import WeylBenchmark
from walsh_ucr.utils.density_matrix import make_rhos
...
def _validate_inputs(args: argparse.Namespace) -> int:
    ...
    if int(args.m_outcome) > max_unique:
        raise ValueError(
            f"m_outcome must be <= 4**n_sys for unique Weyl labels: "
            f"{args.m_outcome} > {max_unique}."
        )
    if int(args.steps) < 1:
